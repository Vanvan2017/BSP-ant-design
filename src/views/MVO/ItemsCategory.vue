...
<!--           <div class="list-content-item">
  ...
  <a-modal v-model="visible" title="Add item" @ok="handleOk">
    Item Title：<a-input :placeholder=title v-model="tempTitle"/> <br /><br />
    Item Category：<a-select :default-value="categoryData[0]" style="width: 300px" v-model="tempCategory">
      <a-select-option v-for="category in categoryData" :key="category">
        {{ category }}
      </a-select-option>
    </a-select><br /><br />
    Item Status：<a-select :default-value="itemStatus[0]" style="width: 300px" v-model="tempStatus">
      <a-select-option v-for="status in itemStatus" :key="status">
        {{ status }}
      </a-select-option>
    </a-select><br /><br />
    <div class="clearfix">
  ...
  <a-modal v-model="visible1" title="Edit item" @ok="handleOk1">
    Item Title：<a-input :placeholder=tempTitle disabled/> <br /><br />
    Item Category：<a-select :default-value="categoryData[0]" style="width: 300px" v-model="tempCategory">
      <a-select-option v-for="category in categoryData" :key="category">
        {{ category }}
      </a-select-option>
...
export default {
  ...
  methods: {
	...
	add() {
    var tempSts;
    if(this.tempStatus=="On Shelf")
      tempSts='A';
    else if(this.tempStatus=='In Repo')
      tempSts='B';
    else
      tempSts='C'
    var tmpuri;
    if(this.fileList.length==0)
      ...
      tmpuri=this.fileList[0].url
		var _this = this
					axios.post('http://localhost:9000/system/mvo/productCategory/insert', {
	...
	update() {
        ...
        if(this.tempStatus=="On Shelf")
          ...
          if(this.fileList.length==0)
            ...
            tmpuri=this.fileList[0].url
	      var _this = this
	              axios.post('http://localhost:9000/system/mvo/productCategory/update', {
